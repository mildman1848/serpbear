...
test: ## Test the Docker image
	...
	@echo "Testing HTTP response (CRITICAL validation)..."
	@sleep 5
	@HTTP_CODE=$$(curl -s -o /dev/null -w "%{http_code}" http://127.0.0.1:1${EXTERNAL_PORT}/ || echo "000"); \
	if [ "$$HTTP_CODE" = "200" ]; then \
		echo "$(GREEN)✓ HTTP response successful ($$HTTP_CODE)$(NC)"; \
	else \
		echo "$(RED)✗ HTTP response failed ($$HTTP_CODE) - CRITICAL$(NC)"; \
		echo "$(RED)This indicates the application is not working properly$(NC)"; \
		$(DOCKER) logs serpbear-test | tail -20; \
		$(DOCKER) stop serpbear-test; \
		exit 1; \
	fi
	@echo "Testing for template variable errors (CRITICAL validation)..."
	@if $(DOCKER) logs serpbear-test 2>&1 | grep -q 'APPLICATION_NAME\|UNDEFINED\|\\$$\\{'; then \
		echo "$(RED)✗ Template variables found in logs - CRITICAL$(NC)"; \
		$(DOCKER) logs serpbear-test | grep -i 'APPLICATION_NAME\|UNDEFINED\|\\$$\\{' | head -5; \
		$(DOCKER) stop serpbear-test; \
		exit 1; \
	else \
		echo "$(GREEN)✓ No template variables found$(NC)"; \
	fi
	@echo "Verifying container is running correctly..."
	@$(DOCKER) inspect serpbear-test >/dev/null || \
		(echo "$(RED)✗ Container is not running$(NC)"; \
		exit 1)
	@echo "$(GREEN)✓ All critical validations passed$(NC)"
